...
"""Release automation script for coiled.

This script automates the release process:
1. Verifies git repo is clean
2. Validates version has -alpha suffix
3. Calculates release version (strips -alpha)
4. Runs validation (lint, tests, pnpm workspace checks)
5. Updates version files (pyproject.toml, __init__.py, widgets/package.json, widgets/src/version.ts)
...
INIT_PY = REPO_ROOT / "src" / "pdum" / "coiled" / "__init__.py"
...
def build_widgets() -> None:
    ...
    run_command([
        "pnpm",
        "--filter",
        "@habemus-papadum/coiled-widgets",
        "build:python",
    ...
    python_bundle_path = "src/pdum/coiled/widgets/index.js"
    python_result = run_command(
        ["git", "status", "--porcelain", python_bundle_path],
        "Checking for Python bundle changes",
        capture_output=True,
    )

    output = f"{result.stdout}{python_result.stdout}"
...
STEPS: list[Step] = [
    ...
    Step(
        ...
        description="Read version from pyproject.toml, __init__.py, widgets/package.json, and widgets/src/version.ts",
        category=StepCategory.VALIDATION,
    ...
    Step(
        ...
        description="Update version to release version in pyproject.toml, __init__.py, widgets/package.json, and widgets/src/version.ts",
...
def main() -> None:
    ...
    # Parse arguments
    parser = argparse.ArgumentParser(description="Automate the release process for coiled")
    parser.add_argument(
        "bump_level",
    ...
    # Show title
    title = Panel.fit(
        "[bold cyan]coiled Release Script[/bold cyan]\n\n"
